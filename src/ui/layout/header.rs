...
use dioxus::{
    desktop::{use_window, WindowCloseBehaviour},
    prelude::*,
};
...
pub fn Header() -> Element {
    ...
    let time_start = use_signal(|| {
        0 * 3600
    });
    let time_end = use_signal(|| {
        24 * 3600
    });
    ...
    rsx! {
        div { class: "fixed top-0 left-0 w-full h-3 flex items-center justify-between px-3 z-200",
            ...
            div { class: "fixed top-1.5 right-1 flex flex-row items-center z-10 gap-1 header-calendar",
                div {
                    onclick: move |evt| {
                    ...
                    div { class: "cursor-pointer rounded-md bg-secondary/20 hover:bg-secondary/40 border border-border/40 px-2.5 py-1 text-xs font-medium backdrop-blur-md select-none transition-all duration-200 hover:scale-[1.02] focus:ring-2 focus:ring-primary focus:ring-offset-1",
                        match show_calendar() {
                            true => {
                    ...
                    if show_calendar() {
                        ...
                        div {
                            ...
                            div { class: "absolute backdrop-blur-md cursor-pointer -top-[32px] left-0 flex gap-1  flex-row  items-center justify-center min-h-[26px] max-h-[26px] rounded-md bg-secondary/20 border border-border/40 text-xs py-0 ",
                                button {
                                    onclick: move |evt| {
                                        evt.stop_propagation();
                                        let naive_next_month = context
                                            .day
                                            .read()
                                            .date_naive()
                            ...
                            EventsCalendar {
                                ...
                                day: day.read().date_naive(),
                                onselect: move |date: chrono::NaiveDate| {
                                    let cl_day = Local
                            ...
                            div { class: "absolute backdrop-blur-md cursor-pointer -bottom-[32px] left-0 flex gap-1  flex-row  items-center justify-center h-[26px]  w-full",
                                if *filter_time.read() {

                                    div { class: "rounded-md glass h-full",
                                    ...
                                    div {
                                        ...
                                        class: "rounded-md glass  p-0.5 aspect-square min-w-[26px] flex items-center justify-center hover:bg-secondary/20 transition-colors",
                                        Icon { icon: LdMinus }
                                    }
                                    div { class: "rounded-md glass  h-full",
                                    ...
                                    div {
                                        ...
                                        class: "rounded-md glass  p-0.5 aspect-square min-w-[26px] flex items0-center justify-center hover:bg-primary/10 transition-colors",
                                        Icon { icon: LdPlus }
                                    }
                                }
                            }
            ...
            JobFormModal {
                ...
                on_save: move |job: JobModel| {
                    ...
                    toast
                        .success(
                            "Задача успешно создана".to_string(),
                            None,
                            None,
                        );
