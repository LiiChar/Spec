      ...
      div {
        ...
        div {
          ...
          div {
            ...
            Icon { icon: LdCalendar, width: 18, height: 18, class: "stroke-foreground/30 group-hover:stroke-foreground" }
          }
        }
        if visible() {
          div {
            class: "absolute min-w-[200px] -bottom-1 translate-y-full flex flex-col gap-1 w-full bg-background p-1 border border-border/30 rounded-lg",
            Button {
                onclick: move |evt: MouseEvent| {
                    evt.stop_propagation();
                    let naive_next_month = current_day.read().checked_sub_months(chrono::Months::new(1)).expect("Failed sub month to current date");
                    let cl_day = Local.from_local_datetime(&naive_next_month.and_hms_opt(0, 0, 0).unwrap()).unwrap().date_naive();
            ...
            Button {
                onclick: move |evt: MouseEvent| {
                    evt.stop_propagation();
                    let naive_next_month = current_day.read().checked_add_months(chrono::Months::new(1)).expect("Failed add month to current date");
                    let cl_day = Local.from_local_datetime(&naive_next_month.and_hms_opt(0, 0, 0).unwrap()).unwrap().date_naive();
                    current_day.set(cl_day);
                    props.onselect.call(cl_day)
                },
                        ...
                        rsx! {
                            div {
                                ...
                                class: format!(
                                    ...
                                    if is_today(date_copy, current_day.read().clone()) { "bg-foreground text-background! shadow-md" } else { "" },
                                    if today == date_copy { "border-[1px] border-foreground" } else { "" },
                                ),
                                tabindex: 0,
                                role: "button",
                                aria_label: format!("Дата: {}, {}", date_copy.format("%d %B %Y"), if is_today(date_copy, current_day.read().clone()) { "сегодня" } else { "" }),
                                "{date_copy.day()}"
                            }
                        }
                    })}
