...
pub fn WindowItem(props: WindowitemProps) -> Element {
    ...
    rsx! {
        div {
            class: "flex justify-between items-center gap-3 p-2 rounded-md hover:bg-secondary/30 transition-colors",
            ...
            div {
                ...
                if let Some(icon) = window.icon_base64.as_deref() {
                    ...
                    div {
                        class: "w-5 h-5 rounded bg-secondary shrink-0"
                    }
                }
            ...
            div {
                            ...
                            spawn(async move {
                                ...
                                match result {
                                    Ok(_) => {
                                        refresh(());
                                        info(
                                            format!("Удалено приложение {}", process.clone()),
                                        );
                                    }
                                    Err(_) => ()
                                }
                        ...
                        DropdownContent {
                            {
                              ...
                              all_tags.read().clone().into_iter().map(move |tag| {
                                        ...
                                        spawn(async move { 
                                          ...
                                          match result {
                                            Ok(_) => {
                                              refresh(());
                                              info(
                                                format!("Добавлен тег {}", tag_name_s),
                                              );
                                            }
                                            Err(_) => ()
                                          }
                ...
                if !tags.is_empty() {
                    div {
                        class: "flex gap-1 shrink-0 flex-wrap justify-end",
