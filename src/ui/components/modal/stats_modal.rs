    ...
    let stats = use_memo({
        ...
        move || {
            if events.is_empty() {
                ...
                Some(event_stats(events.clone()))
            }
        }
    });

    let first_event = props.events.first();
    let last_event = props.events.last();

    let formatted_start = if props.start_ts == 0 {
        first_event
            .map(|e| convert_ts_to_local_date(e.timestamp as u64).format("%H:%M:%S").to_string())
            .unwrap_or_else(|| "--:--:--".to_string())
    } else {
        convert_ts_to_local_date(props.start_ts as u64)
            .format("%H:%M:%S")
            .to_string()
    };

    let formatted_end = if props.end_ts == 0 {
        last_event
            .map(|e| convert_ts_to_local_date(e.timestamp as u64).format("%H:%M:%S").to_string())
            .unwrap_or_else(|| "--:--:--".to_string())
    } else {
        convert_ts_to_local_date(props.end_ts as u64)
            .format("%H:%M:%S")
            .to_string()
    };
    ...
    rsx! {
        div {
            ...
            div {
                ...
                // шапка модалки: время
                div {
                    class: "flex items-center gap-2 mb-2 justify-between border-b border-border/40 pb-1",
                    div {
                ...
                if props.events.is_empty() {
                    div {
                        ...
                        div { class: "text-lg font-medium", "Нет событий" }
                        div { class: "text-sm opacity-70 mt-1", "За выбранный период данные отсутствуют" }
                    }
                } else {
                    div {
                            ...
                            rsx! {
                                ...
                                div { class: "grid grid-cols-2 gap-2 text-sm",
                                    ...
                                    div { class: "rounded-md border border-border/40 p-2",
                                        ...
                                        div { class: "font-medium", "{format_duration_short(s.active_time)} ({format_active_percent})" }
                                    }
                                    div { class: "rounded-md border border-border/40 p-2",
                               ...
                               { 
                                if let Some(app) = &s.most_used_app {
                                    ...
                                    rsx! {
                                        div {
                                            ...
                                            div { class: "text-sm opacity-80", "{format_duration_short(app.total_time)} · {act_per} активного времени" }
                                        }
                                    }
                                } else {
                                  rsx! { "" }
                                ...
                                if !s.app_list.is_empty() {
                                    div { class: "flex flex-col gap-1 max-h-[40vh]",
                                        for app in s.app_list.iter().take(5) {
                                            div { class: "rounded-md border border-border/40 p-2",
                                                div { class: "flex justify-between gap-2",
                                                    div { class: "truncate text-sm font-medium", "{app.name}" }
                                                    div { class: "text-xs opacity-70 whitespace-nowrap", "{format_duration_short(app.total_time)}" }
                                                }
                                                div { class: "mt-1 h-1.5 rounded-full bg-primary/40 overflow-hidden",
                                                    div { class: "h-full rounded-full bg-primary", style: format!("width: {:.2}%", app.active_percent) }
                                                }
                                            }
                                        }
                                    }
